...
export async function GET(
  ...
  try {
    ...
    const res = await clientRequest(`/servers/${identifier}/resources`)

    if (!res.ok) {
      const text = await res.text()
      console.error("Failed to fetch resources:", text)
      return NextResponse.json({ error: "Failed to fetch resources" }, { status: res.status })
    }
